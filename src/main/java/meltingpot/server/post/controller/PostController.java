...
public class PostController {
    ...
    @PostMapping("")
    public ResponseEntity<ResponseData> createPost(  @CurrentUser Account account,@RequestBody PostRequestDTO.CreatePostDTO createPostDTO) {
        postService.createPost(createPostDTO,account);
        try{
            return ResponseData.toResponseEntity(ResponseCode.CREATE_POST_SUCCESS);
        }catch (NoSuchElementException e) {
            return ResponseData.toResponseEntity(ResponseCode.POST_NOT_FOUND);
        }
    }
